"""This function generates an assignment by creating random clusters of robots and tasks
   Clusters are more likely to merge with clusters that they capability match with.
Inputs:
...
from phase1.generate_clusters import generate_clusters_heuristic
from phase2.IP_assignment import IP_assignment
import copy
import time

def cluster_assignment_heuristic(robot_list, task_list, hypes, max_iterations=10000, time_limit=5.0, printout=False):
    ...
    while time.time() - start_time < time_limit and iteration < max_iterations:
        ...
        clusters = generate_clusters_heuristic(assignment, robot_list, task_list, temp, hypes)
